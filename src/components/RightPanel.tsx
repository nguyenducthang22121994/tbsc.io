...
import { MoreVertical, History, RotateCw, ChevronDown, ArrowUpDown, Plus, Pencil, Trash2 } from 'lucide-react';
...
const RightPanel = () => {
  ...
  const handleSaveStock = (stock: StockData) => {
    if (editingIndex !== null) {
      setStockData(prev => {
        const newData = [...prev];
        newData[editingIndex] = stock;
        return newData;
      });
  ...
  const [stockData, setStockData] = useState<StockData[]>([
    ...
    { symbol: 'VND', totalQty: 10000, availableQty: 6000, avgPrice: 22.62, marketPrice: 19.00, marketPriceChange: -1.6, pendingBuy: 0, marketValue: 190000000, costValue: 226200000, profitPercent: -16.0, profitValue: -36200000 },
    ...
    { symbol: 'SSI', totalQty: 5000, availableQty: 5000, avgPrice: 35.65, marketPrice: 29.4, marketPriceChange: -1.7, pendingBuy: 0, marketValue: 161250000, costValue: 178250000, profitPercent: -9.0, profitValue: -16000000 },
    { symbol: 'TCB', totalQty: 3000, availableQty: 3000, avgPrice: 39.56, marketPrice: 34.70, marketPriceChange: -2.0, pendingBuy: 0, marketValue: 104100000, costValue: 118688000, profitPercent: -12.3, profitValue: -14588000 },
    ...
    { symbol: 'SHS', totalQty: 19500, availableQty: 13000, avgPrice: 23.8, marketPrice: 21.60, marketPriceChange: -1.8, pendingBuy: 6500, marketValue: 421200000, costValue: 455910000, profitPercent: -7.6, profitValue: -34710000 },
  ...
  const totalMarketValue = stockData.reduce((sum, item) => sum + item.marketValue, 0);
  const totalCostValue = stockData.reduce((sum, item) => sum + item.costValue, 0);
  const totalProfitValue = totalMarketValue - totalCostValue;
  const totalProfitPercent = totalCostValue !== 0 ? (totalProfitValue / totalCostValue) * 100 : 0;
  ...
  return (
    <div className="w-full lg:w-2/3 bg-[var(--card-bg)] rounded-lg flex flex-col h-full border border-[var(--card-border)]">
        ...
        <div className="flex flex-wrap justify-between items-end">
          <div className="flex items-center space-x-6 mb-1 lg:mb-0">
            ...
            <div>
              ...
              <div className="text-xs text-white font-bold">{formatCurrency(totalCostValue)}</div>
            </div>
            <div>
              <div className="text-[10px] text-[var(--text-muted)] mb-0.5">Lợi nhuận</div>
              <div className={clsx("text-xs font-bold", totalProfitValue >= 0 ? "text-[var(--success)]" : "text-[var(--danger)]")}>
                {formatCurrency(totalProfitValue)} ({totalProfitPercent.toFixed(1)}%)
              </div>
            </div>
            <div>
              <div className="text-[10px] text-[var(--text-muted)] mb-0.5">Tiền mặt</div>
              <div className="text-xs text-white font-bold">160,045,023</div>
            </div>
          </div>
            ...
            >
              <Plus className="w-3 h-3 mr-1" /> Thêm mã
            </div>
      ...
      <AddStockModal 
        ...
        initialData={editingIndex !== null ? stockData[editingIndex] : undefined}
      />
        ...
        <table className="w-full text-right border-collapse">
          ...
          <tbody className="text-xs">
            ...
            <tr className="border-b border-[var(--card-border)] font-bold text-white bg-[#1e1e1e]">
              ...
              <td className="p-2 border-r border-[var(--card-border)]"></td>
              <td className="p-2 border-r border-[var(--card-border)]"></td>
              <td className="p-2 border-r border-[var(--card-border)]">{formatCurrency(totalMarketValue)}</td>
              <td className={clsx("p-2 border-r border-[var(--card-border)]", totalProfitPercent >= 0 ? "text-[var(--success)]" : "text-[var(--danger)]")}>
                {totalProfitPercent > 0 ? '+' : ''}{totalProfitPercent.toFixed(1)}%
              </td>
              <td className={clsx("p-2 border-r border-[var(--card-border)]", totalProfitValue >= 0 ? "text-[var(--success)]" : "text-[var(--danger)]")}>
            ...
            {stockData.map((stock, index) => (
              <tr key={index} className="border-b border-[#2a2a2a] hover:bg-[#2a2a2a] transition-colors group">
                ...
                <td className="p-2 text-[var(--text-muted)] flex justify-end items-center space-x-2">
                  <Pencil 
                    className="w-3 h-3 cursor-pointer hover:text-white" 
                    onClick={() => openEditModal(index)}
                  />
                  <Trash2 
                    className="w-3 h-3 cursor-pointer hover:text-[var(--danger)]" 
                    onClick={() => handleDeleteStock(index)}
                  />
                  <MoreVertical className="w-3 h-3 cursor-pointer hover:text-white" />
                </td>
              </tr>
            ))}
          </tbody>
